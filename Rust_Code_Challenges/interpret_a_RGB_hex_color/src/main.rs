...
impl FromStr for Rgb {  
    ...
    fn from_str(s: &str) -> Result<Self,Self::Err>{
        if let Some(hex) = s.trim().strip_prefix('#'){
            ...
            Ok(Rgb{
                r:r.or_else(|_err|Err(ParseColorError::RedOutOfBound))?,
                g:g.or_else(|_err|Err(ParseColorError::GreenOutOfBound))?,
                b:b.or_else(|_err|Err(ParseColorError::BlueOutOfBound))?,
            })
